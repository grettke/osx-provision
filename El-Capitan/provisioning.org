...
Set the computer names [[http://hack.org/mc/writings/mac-survival.html][via]]
#+BEGIN_QUOTE
On a fresh install a Mac sets your hostname to something boring like
“Michaels-MacBookAir”. To change it, use:

% sudo scutil --set HostName jsb.hack.org
% sudo scutil --set LocalHostName jsb
% sudo scutil --set ComputerName "Johann Sebastian Bach"
“HostName” is the ordinary hostname. “LocalHostName” is the Bonjour host name.
”ComputerName” is a user friendly version of the above.
#+END_QUOTE

#+NAME: 291FDE06-DF44-4156-A013-B763A8727B00
#+BEGIN_SRC shell
echo "Enter technie hostname, needs at least two parts: "
read vhn
echo "Enter Bonjourie Bonjour name, needs only one part: "
read vbn
echo "Enter friendly computer name, full words: "
read vfn
sudo scutil --set HostName $vhn
sudo scutil --set LocalHostName $vbn
sudo scutil --set ComputerName $vfn
#+END_SRC

I used:
- hostname :: box-uuid.vm
- bonjour :: box-uuid
- computer :: box-name (box-uuid)
