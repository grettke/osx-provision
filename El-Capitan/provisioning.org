...
- HostName
  ...
  - SLD
    - For virtual machines :: UUID for the node
    - For everything else :: Logical name for the node
- LocalHostName
  - For virtual machines :: UUID for the node
  - For everything else :: Logical name for the node
- ComputerName
  - Single string composed of the logical name followed by a space and then
    the UUID for the node in round parentheses
