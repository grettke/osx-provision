...
- Make VMWare Disk Drive sized in multiples of 8
  - That way they can be easily shared and transferred on SD cards
- VMWare Fusion does not pre-allocate disk space and doesn't give you the option
  to do so
  - This is an option on VMWare Workstation so you might be surprised or
    confused when you look at file size on the host
