...
- Do not set up the "Compare with DeltaWalker.workflow"
  - It *was* done by following the directions
  ...
  - No longer do that because I have not figured out why it broke, and it brewaks
    for Git, and Git is more important 99% of the time usage vs direct file
    comparisons

- Manually compare two of the test files to see that it runs and font's look
  right
